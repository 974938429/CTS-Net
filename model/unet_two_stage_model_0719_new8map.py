...
class UNetStage2(nn.Module):
    # 根据一阶段的输出与输入src的堆叠  二阶段 的n_channels 变化
    def __init__(self, n_channels=4, bilinear=False):
        ...
        self.relation1 = RelationMap(in_channels=64, out_channels=1)  # 64-》对应的两通道子图
        self.relation2 = RelationMap(in_channels=64, out_channels=1)
        self.relation3 = RelationMap(in_channels=64, out_channels=1)
        self.relation4 = RelationMap(in_channels=64, out_channels=1)

        self.relation5 = RelationMap(in_channels=64, out_channels=1)
        self.relation6 = RelationMap(in_channels=64, out_channels=1)
        self.relation7 = RelationMap(in_channels=64, out_channels=1)
        self.relation8 = RelationMap(in_channels=64, out_channels=1)
        ...
        self.final = RelationFuse(in_channels=8 + 8, out_channels=1)  # 24层 -》1层
    ...
    def forward(self, x, stage3, stage2, stage1):
        ...
        r1 = self.relation1(x_up4)
        r2 = self.relation2(x_up4)

        r3 = self.relation3(x_up4)
        r4 = self.relation4(x_up4)

        r5 = self.relation5(x_up4)
        r6 = self.relation6(x_up4)

        r7 = self.relation7(x_up4)
        r8 = self.relation8(x_up4)

        with_r = self.with_relation(x_up4)
        # conv -bn -relu
        x = torch.cat([r1, r2, r3, r4, r5, r6, r7, r8, with_r], dim=1)
        # x=8+8=16层  生成的8邻域现在为单通道图
        # 输出与8子图的数值有关  8张子图和输出都有loss约束
        x = self.final(x)
        # 8+8  ->1   边缘非边缘

        return [x, r1, r2, r3, r4, r5, r6, r7, r8,x_up4]
        # 9张图 1+8张子图   x 为单通道    子图为8张的2通道

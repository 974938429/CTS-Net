...
parser.add_argument('--resume', default=[
    '../parameters/the_coarse_stage_with_info_loss.pth',
    '../parameters/the_refined_stage_with_info_loss.pth'
], type=list, metavar='PATH',
                    help='path to latest checkpoint (default: none)')
parser.add_argument('--src_path',default='../samples/coverage')
parser.add_argument('--save_path',default='../results/coverage')
...
def main():
    ...
    if os.path.exists(args.src_path):
        for index,item in enumerate(os.listdir(args.src_path)):
            ...
            plt.title('The prediction of the coarse stage.')
            plt.subplot(1, 2, 2)
            plt.imshow(pred_mask_2)
            plt.title('The prediction of the refined stage.')
            plt.show()
            plt.clf()
